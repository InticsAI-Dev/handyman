...
public class AuthIdValidator {
    ...
    private void processAuthAndMemberIds(Map data, Long rootPipelineId) {
        ...
        if(authIdObj instanceof List && memberIdObj instanceof List)
        {
            ...
            PostProcessingExecutorInput authIdInput = null;
            ...
            for(int i = 0; i<authIdList.size(); i++)
            {
                ...
                {
                    authIdInput = (PostProcessingExecutorInput) obj;
                    String authId = authIdInput.getExtractedValue();
                    if (!authId.equals("")) {
                        String validatedAuthId = processAuthIds(authId, uniqueAuthIds, validAuthIds, rootPipelineId);
                        authIdInput.setExtractedValue(validatedAuthId);
                    }
                }
            }
            ...
            for(int i = 0; i<memberIdList.size(); i++)
            {
                ...
                if(obj instanceof PostProcessingExecutorInput)
                {
                    PostProcessingExecutorInput memberIdInput = (PostProcessingExecutorInput) obj;
                    String memberId = memberIdInput.getExtractedValue();
                    ...
                    if (!memberId.equals("")) {
                        ...
                        if (!validUmIds.isEmpty()) {
                            ...
                            authIdInput = new PostProcessingExecutorInput();
                            authIdInput.setExtractedValue(validAuthIds.isEmpty() ? "" : join(validAuthIds, ","));
                            authIdList.add(authIdInput);
                            memberIdInput.setExtractedValue("");
                        } else if (!memberId.toUpperCase().startsWith("UM")){
                            ...
                            memberIdInput.setExtractedValue(validMemberId);
                        }
                    }
                    else {
                        memberIdInput.setExtractedValue("");
    ...
    private void processMemberIds(String memberId, HashSet uniqueAuthIds, List validAuthIds, Long rootPipelineId, List validUmIds)
    {
        ...
        if (!memberId.equals("")) {
            ...
            if (memberId.toUpperCase().startsWith("UM")) {
