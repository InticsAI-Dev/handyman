...
public class MemberDOBandServiceFromDateMapper {

    private Logger logger;

    public MemberDOBandServiceFromDateMapper(Logger logger) {
        this.logger = logger;
    }
    ...
    public MappingResult doCustomPredictionMapping(Map predictionKeyMap, Long rootPipelineId) {
        ...
        Object dobObj = predictionKeyMap.get("member_date_of_birth");
        if (dobObj instanceof List) {
            List dobList = (List) dobObj;

            for (int i = 0; i < dobList.size(); i++) {
                Object obj = dobList.get(i);
                if (obj instanceof PostProcessingExecutorInput) {

                    PostProcessingExecutorInput dobInput =
                            (PostProcessingExecutorInput) obj;

                    String validatedDob =
                            dateValidator(dobInput.getExtractedValue(),
                                    rootPipelineId,
                                    true);

                    dobInput.setExtractedValue(validatedDob);
                    if(validatedDob.isEmpty()){
                        dobInput.setLabel("");
                        dobInput.setSectionAlias("");
                        dobInput.setBBox("");
                    }
                }
            }
            predictionKeyMap.put("member_date_of_birth", dobList);
        }

        Object serviceObj = predictionKeyMap.get("service_from_date");
        if (serviceObj instanceof List) {
            ...
            for (int i = 0; i < serviceList.size(); i++) {
                ...
                if (obj instanceof PostProcessingExecutorInput) {
                    ...
                    PostProcessingExecutorInput serviceInput =
                            (PostProcessingExecutorInput) obj;

                    String validatedServiceDate =
                            dateValidator(serviceInput.getExtractedValue(),
                                    rootPipelineId,
                                    false);

                    serviceInput.setExtractedValue(validatedServiceDate);
                    if(validatedServiceDate.isEmpty()){
                        serviceInput.setLabel("");
                        serviceInput.setSectionAlias("");
                        serviceInput.setBBox("");
                    }
                }
            }
            predictionKeyMap.put("service_from_date", serviceList);
        }
    ...
    public Date deductCentury(Date inputDate, int currentYear, String logPrefix) {
        ...
        if (parsedYear > currentYear) {
            ...
            logger.info(logPrefix + "'member_date_of_birth' deducted 100 years.");
        }
