        ...
        if (valueObj instanceof List) {
            ...
            for (int i = 0; i < objList.size(); i++) {
                ...
                if (obj instanceof PostProcessingExecutorInput) {
                    ...
                    String value = medicaidIdInput.getExtractedValue() != null ? medicaidIdInput.getExtractedValue().trim() : "";
                    ...
                    if (parts.length > 1) {
                        ...
                        if (firstPart.toUpperCase().startsWith("UM")) {
                            if (UM_PATTERN.matcher(firstPart).matches()) {
                                objAuthIdListAdd(resultMap, medicaidIdInput, firstPart, fieldName);
                                updateMemberIdObj(resultMap,"member_id", "");

                            } else {
                                updateMemberIdObj(resultMap,"member_id", "");
                            }
                        } else {
                            valueCheck(resultMap, medicaidIdInput, fieldName, firstPart);
                            updateMemberIdObj(resultMap,"member_id", firstPart);
...
//                                resultMap.put("auth_id", secondPart);
                                objAuthIdListAdd(resultMap, medicaidIdInput, secondPart, "auth_id");
...
//                                additionalAuthCandidates.add(secondPart);
    ...
    private void valueCheck(Map resultMap, PostProcessingExecutorInput inputObj, String fieldName, String value) {
        ...
        if (value.toUpperCase().startsWith("UM")) {
            if (UM_PATTERN.matcher(value).matches()) {
                objAuthIdListAdd(resultMap, inputObj, value, fieldName);
                ...
                if (obj instanceof PostProcessingExecutorInput) {
                    ...
                    if (authId != null && authId.length() != 0) {
                        if (UM_PATTERN.matcher(authId).matches()) {
                            authIdInput.setExtractedValue(authId);
                    ...
                    if (!authIdCandidates.isEmpty()) {
                        ...
                        authIdInput.setExtractedValue(firstAuthId);
