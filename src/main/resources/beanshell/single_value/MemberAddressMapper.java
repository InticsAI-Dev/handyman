...
public class MemberAddressMapper {
    ...
    public MappingResult doCustomPredictionMapping(Map predictionKeyMap, Long rootPipelineId) {
        ...
        PostProcessingExecutorInput addressLine1Obj = getFirst(predictionKeyMap, "member_address_line1");
        PostProcessingExecutorInput cityObj = getFirst(predictionKeyMap, "member_city");
        PostProcessingExecutorInput stateObj = getFirst(predictionKeyMap, "member_state");
        PostProcessingExecutorInput zipcodeObj = getFirst(predictionKeyMap, "member_zipcode");

        String inputAddress = hasValue(addressLine1Obj) ? addressLine1Obj.getExtractedValue().trim() : 
                (addressLine1Obj != null && addressLine1Obj.getExtractedValue() == null ? null : "");
        String city = hasValue(cityObj) ? cityObj.getExtractedValue().trim() : "";
        String state = hasValue(stateObj) ? stateObj.getExtractedValue().trim() : "";
        String zipcode = hasValue(zipcodeObj) ? zipcodeObj.getExtractedValue().trim() : "";
        ...
        if (inputAddress != null && !inputAddress.isEmpty()) {
            logger.info(logPrefix + "Processing ''member_address_line1'' field (Input type: List<PostProcessingExecutorInput>).");
