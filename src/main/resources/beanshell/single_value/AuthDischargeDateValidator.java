...
public class AuthDischargeDateValidator {

    private Logger logger;

    public AuthDischargeDateValidator(Logger logger) {
        this.logger = logger;
    }
    ...
    public MappingResult doCustomPredictionMapping(Map predictionKeyMap, Long rootPipelineId) {
        ...
        logger.info(logPrefix + "Entered AuthDischargeDateValidator.doCustomPredictionMapping method.");

        if (predictionKeyMap == null) {
            return new MappingResult(new HashMap());
        }

        Object serviceDateObj = predictionKeyMap.get("auth_discharge_date");
        if (serviceDateObj instanceof List) {
            List serviceList = (List) serviceDateObj;

            for (int i = 0; i < serviceList.size(); i++) {
                Object obj = serviceList.get(i);
                if (obj instanceof AuthDischargeDateValidator.PostProcessingExecutorInput) {

                    AuthDischargeDateValidator.PostProcessingExecutorInput authDischargeDate =
                            (AuthDischargeDateValidator.PostProcessingExecutorInput) obj;
                    if (authDischargeDate.getExtractedValue() != null && !authDischargeDate.getExtractedValue().isEmpty()) {
                        String validatedAuthDischargeDate =
                                DateValidator(authDischargeDate.getExtractedValue(),
                                        rootPipelineId,
                                        false);

                        authDischargeDate.setExtractedValue(validatedAuthDischargeDate);
                        if (validatedAuthDischargeDate.isEmpty()) {
                            authDischargeDate.setLabel("");
                            authDischargeDate.setSectionAlias("");
                            authDischargeDate.setBBox("");
                        }
                    }
                }
            }
            predictionKeyMap.put("auth_discharge_date", serviceList);
        } else {
            logger.warn(logPrefix + "'auth_discharge_date' is not a List. Skipping processing.");
        }
        ...
        if (dateValue instanceof String) {
            ...
            if (!inputDate.isEmpty()) {
                ...
                if (parsedDate != null) {
                    ...
                    logger.warn(logPrefix + "Date mapping failed for 'AuthDischargeDateValidator'. Removed based on configuration.");
                    return "";
                }
            } else {
