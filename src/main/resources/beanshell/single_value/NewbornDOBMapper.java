...
public class NewbornDOBMapper {
    ...
    public NewbornDOBMapper(Logger logger) { this.logger = logger; }
    ...
    public MappingResult doCustomPredictionMapping(Map predictionKeyMap, Long rootPipelineId) {
        ...
        logger.info(logPrefix + "Entered newbornDOBMapper.doCustomPredictionMapping method.");

        if (predictionKeyMap == null) {
            return new MappingResult(new java.util.HashMap());
        }

        Object newbornDobObj = predictionKeyMap.get("newborn_date_of_birth");

        if(newbornDobObj instanceof List)
        {
            List newbornDobList = (List) newbornDobObj;
            for(int i = 0; i<newbornDobList.size(); i++)
            {
                Object obj = newbornDobList.get(i);
                if(obj instanceof PostProcessingExecutorInput)
                {
                    PostProcessingExecutorInput newbornDobInput = (PostProcessingExecutorInput) obj;
                    String validatedDob = DateValidator(newbornDobInput.getExtractedValue(), rootPipelineId, true);
                    newbornDobInput.setExtractedValue(validatedDob);
                }
            }
            predictionKeyMap.put("newborn_date_of_birth", newbornDobList);
        }
        ...
        if (dateValue instanceof String) {
            ...
            if (!inputDate.isEmpty()) {
                ...
                if (parsedDate != null) {
                    ...
                    logger.warn(logPrefix + "Date mapping failed for 'newborn_date_of_birth'. Removed based on configuration.");
                    return "";
                }
            } else {
    ...
    public Date deductCentury(Date inputDate, int currentYear, String logPrefix) {
        ...
        if (parsedYear > currentYear) {
            ...
            logger.info(logPrefix + "'newborn_date_of_birth' deducted 100 years.");
        }
