...
public class HwDetectionAction implements IActionExecution {
    ...
    public static String httpClientTimeout;

    public HwDetectionAction(final ActionExecutionAudit action, final Logger log,
                             final Object hwDetection) {
        HwDetectionAction.hwDetection = (HwDetection) hwDetection;
        this.action = action;
        this.log = log;
        httpClientTimeout = action.getContext().get(OKHTTP_CLIENT_TIMEOUT);
        this.aMarker = MarkerFactory.getMarker(" HwDetection:" + HwDetectionAction.hwDetection.getName());
    }
    ...
    public void execute() throws Exception {
        ...
        try {
            ...
            log.info("paper classification read batch size {} and queryset from macro {} ", readBatchSize, hwDetection.getQuerySet());
            Thread.sleep(1000);

            coproProcessor.startConsumer(insertQuery, consumerCount, writeBatchSize, hwClassificationConsumerProcess);
            log.info(aMarker, "Paper classification has been completed {}  ", hwDetection.getName());
        } catch (Exception e) {
            action.getContext().put(hwDetection.getName() + ".isSuccessful", "false");
            log.error(aMarker, "Error at paper classification execute method {}", ExceptionUtil.toString(e));
            throw new HandymanException("Error at paper classification execute method ", e, action);
        }
    }
