 ...
 */
@ActionExecution(actionName = "TableExtraction")
public class TableExtractionAction implements IActionExecution {
    ...
    public void execute() {
        try {
            ...
            final String insertQuery = "INSERT INTO " + tableExtraction.getResultTable() +
                    "(origin_id, group_id, root_pipeline_id, tenant_id, page_number, markdown_table,stage, status, model_name, "
                    +
            ...
            final List<URL> urls = Optional.ofNullable(tableExtraction.getEndpoint())
            ...
            final CoproProcessor<TableExtractionInputTable, TableExtractionOutputTable> coproProcessor = new CoproProcessor<>(
            ...
            coproProcessor.startProducer(tableExtraction.getQuerySet(),
                    Integer.valueOf(action.getContext().get(DB_SELECT_READ_BATCH_SIZE)));
            log.info(aMarker, "table extraction copro coproProcessor startProducer called read batch size {}",
                    action.getContext().get(DB_SELECT_READ_BATCH_SIZE));
            Thread.sleep(1000);
            coproProcessor.startConsumer(insertQuery,
                    Integer.valueOf(action.getContext().get("table.extraction.consumer.API.count")),
                    Integer.valueOf(action.getContext().get(DB_INSERT_WRITE_BATCH_SIZE)),
                    new TableExtractionConsumerProcess(log, aMarker, action));
            log.info(aMarker,
                    "table extraction copro coproProcessor startConsumer called consumer count {} write batch count {} ",
                    Integer.valueOf(action.getContext().get("table.extraction.consumer.API.count")),
                    Integer.valueOf(action.getContext().get(DB_INSERT_WRITE_BATCH_SIZE)));
    ...
    // private static void extractedOutputResponse(String responseParse,
    // List<TableExtractionOutputTable> parentObj, String originId, Integer groupId,
    // String templateId, Long tenantId, Long processId, String
    // tableExtractionProcessName, Long rootPipelineId) {
    // JSONObject parentResponse = new JSONObject(responseParse);
    // JSONArray filePathArray = new
    // JSONArray(parentResponse.get("csvTablesPath").toString());
    //
    // //JSONArray tableResponseArray = new
    // JSONArray(parentResponse.get("table_response").toString());
    // log.info(aMarker, "coproProcessor consumer process response body
    ...
    // }
    // log.info(aMarker, "coproProcessor consumer process response body
    // processedJsonNode {}", processedJsonNode);
    //
    // processedJsonNode.forEach((schemaName, csvFilePathNode) -> {
    // try {
    // log.info(aMarker, "coproProcessor consumer process response body scheme {}
    // and csv file path {}", schemaName, csvFilePathNode);
    //
