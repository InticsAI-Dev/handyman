...
public class FtpDownloadAction implements IActionExecution {
    ...
    public void execute() throws Exception {
        ...
        try {
            ...
            downloadSingleFileOrFolder(ftpClient, sourceDir, DestDir, downloadedFilePaths);
                ...
                if (subFiles != null) {
                    for (FTPFile aFile : subFiles) {
                        ...
                        if (lastModifiedTime.isBefore(uploadTimeDate)) {
                            // If uploadTime is specified and the file was uploaded after that time, download the file
                            downloadSingleFileOrFolder(ftpClient, sourceDir, DestDir, downloadedFilePaths);
                            if (!downloadedFilePaths.isEmpty()) {
                                ObjectMapper mapper = new ObjectMapper();
                                String filePaths = mapper.writeValueAsString(downloadedFilePaths);
...
public void downloadSingleFileOrFolder(FTPClient ftpClient, String remotePath, String saveDir, List<String> downloadedFilePaths) throws IOException {
    FTPFile[] subFiles = ftpClient.listFiles(remotePath);

    if (subFiles != null && subFiles.length > 0) {
        FTPFile firstFile = subFiles[0];  // Assuming the first element in the array represents the file or directory
        if (firstFile.isDirectory()) {
            downloadDirectory(ftpClient, remotePath, saveDir, downloadedFilePaths);
        } else {
            downloadSingleFile(ftpClient, remotePath, saveDir, downloadedFilePaths);
        }
    } else {
        // Handle the case where subFiles is null or empty (path doesn't exist or other error)
        log.error("No files or directories found at: {}", remotePath);
    }
}


    public void downloadSingleFile(FTPClient ftpClient, String remoteFilePath, String saveDir, List<String> downloadedFilePaths) throws IOException {
        File downloadFile = new File(saveDir + File.separator + new File(remoteFilePath).getName());
        ...
        try (OutputStream outputStream = new BufferedOutputStream(new FileOutputStream(downloadFile))) {
            ftpClient.setFileType(FTP.BINARY_FILE_TYPE);
            log.info(aMarker, "Download file {} ", downloadFile.getAbsolutePath());
            boolean success = ftpClient.retrieveFile(remoteFilePath, outputStream);

            if (success) {
                log.info(aMarker, "Downloaded the file {} ", remoteFilePath);
    ...
    public void downloadDirectory(FTPClient ftpClient, String remoteFolderPath, String saveDir, List<String> downloadedFilePaths) throws IOException {
        FTPFile[] subFiles = ftpClient.listFiles(remoteFolderPath);
        if (subFiles != null) {
            for (FTPFile aFile : subFiles) {
                String currentFileName = aFile.getName();
                if (currentFileName.equals(".") || currentFileName.equals("..")) continue;
                String remoteFilePath = remoteFolderPath + "/" + currentFileName;
                String savePath = saveDir + File.separator + currentFileName;

                if (aFile.isFile()) {
                    downloadSingleFile(ftpClient, remoteFilePath, saveDir, downloadedFilePaths);
                } else {
                    downloadDirectory(ftpClient, remoteFilePath, savePath, downloadedFilePaths);
                }
            }
        }
    }
