...
import in.handyman.raven.lib.services.neradaptors.NerAdapterConsumerProcess;
import in.handyman.raven.lib.services.neradaptors.NerInputTable;
import in.handyman.raven.lib.services.neradaptors.NerOutputTable;
...
public class NerAdapterAction implements IActionExecution {
    ...
    public void execute() throws Exception {
        ...
        try {
            ...
            coproProcessor.startProducer(nerAdapter.getResultSet(), Integer.valueOf(action.getContext().get(DB_SELECT_READ_BATCH_SIZE)));
            log.info(aMarker, "ner adapter copro coproProcessor startProducer called read batch size {}", action.getContext().get(DB_SELECT_READ_BATCH_SIZE));
            Thread.sleep(1000);
            coproProcessor.startConsumer(insertQuery, Integer.valueOf(action.getContext().get("ner.consumer.API.count")), Integer.valueOf(action.getContext().get(DB_INSERT_WRITE_BATCH_SIZE)),
                    new NerAdapterConsumerProcess(log, aMarker, action));
            log.info(aMarker, "ner adapter copro coproProcessor startConsumer called consumer count {} write batch count {} ", Integer.valueOf(action.getContext().get("ner.consumer.API.count")), Integer.valueOf(action.getContext().get(DB_INSERT_WRITE_BATCH_SIZE)));
