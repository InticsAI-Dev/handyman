...
//     this.URI = action.getContext().get("copro.masterdata-comparison.url");
  ...
  public void execute() throws Exception {
    ...
    try {
      ...
      String masterdataComparisonEndPoint = masterdataComparison.getEndPoint();
      final List<URL> urls = Optional.ofNullable(action.getContext().get(masterdataComparisonEndPoint)).map(s -> Arrays.stream(s.split(",")).map(s1 -> {
        try {
          return new URL(s1);
      ...
      coproProcessor.startConsumer(insertQuery, Integer.valueOf(action.getContext().get("consumer.masterdata.API.count")), Integer.valueOf(action.getContext().get("write.batch.size")),
              new MasterdataComparisonProcess(log, aMarker, action), masterdataComparisonEndPoint);
