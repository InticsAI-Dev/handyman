...
public class ConsumerAction implements IActionExecution {
  ...
  public void execute() {
    ...
    while (true) {
      ...
      final String sourceDb = consumer.getSource();
      final Jdbi jdbi = ResourceAccess.rdbmsJDBIConn(sourceDb);

      final List<Map<String, Object>> maps=  jdbi.withHandle(handle -> handle.createQuery(event)
                      .mapToMap().list());
