...
public class BlankPageRemoverAction implements IActionExecution {
    ...
    public void execute() throws Exception {
        try {
            ...
            coproProcessor.startProducer(blankPageRemover.getQuerySet(), Integer.valueOf(action.getContext().get(DB_SELECT_READ_BATCH_SIZE)));
            Thread.sleep(1000);
            //8. call the method start consumer from coproprocessor
            coproProcessor.startConsumer(insertQuery, Integer.valueOf(action.getContext().get("blank.page.removal.consumer.API.count")), Integer.valueOf(action.getContext().get(DB_INSERT_WRITE_BATCH_SIZE)), new BlankPageRemoverAction.BlankPageRemoverConsumerProcess(log, aMarker, action, outputDir));
