...
public class RadonKvpAction implements IActionExecution {
  ...
  public void execute() throws Exception {
    try {
      ...
      final List<URL> urls = Optional.ofNullable(action.getContext().get(radonKvpUrl)).map(s -> Arrays.stream(s.split(",")).map(urlItem -> {
        try {
      ...
      coproProcessor.startConsumer(insertQuery, consumerApiCount, writeBatchSize, radonKvpConsumerProcess, radonKvpUrl);
