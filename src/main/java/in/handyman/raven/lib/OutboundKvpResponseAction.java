...
public class OutboundKvpResponseAction implements IActionExecution {
  ...
  public void execute() throws Exception {
    ...
    try {
      ...
      coproProcessor.startProducer(outboundKvpResponse.getQuerySet(), Integer.valueOf(action.getContext().get(DB_SELECT_READ_BATCH_SIZE)));
      ...
      Thread.sleep(1000);
      coproProcessor.startConsumer(insertQuery, Integer.valueOf(action.getContext().get("alchemy.kvp.consumer.API.count")), Integer.valueOf(action.getContext().get(DB_INSERT_WRITE_BATCH_SIZE)), new OutboundKvpConsumerProcess(log,aMarker,action,this));
      log.info(aMarker, "product outbound coproProcessor startConsumer called consumer count {} write batch count {} ",Integer.valueOf(action.getContext().get("alchemy.kvp.consumer.API.count")),Integer.valueOf(action.getContext().get(DB_INSERT_WRITE_BATCH_SIZE)));
