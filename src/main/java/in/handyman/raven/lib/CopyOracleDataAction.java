...
public class CopyOracleDataAction implements IActionExecution {
  private final ActionExecutionAudit actionExecutionAudit;

  private final Logger log;

  private final CopyOracleData copyOracleData;

  private final Marker aMarker;

  public CopyOracleDataAction(final ActionExecutionAudit action, final Logger log,
      final Object copyOracleData) {
    this.copyOracleData = (CopyOracleData) copyOracleData;
    this.actionExecutionAudit = action;
    this.log = log;
    this.aMarker = MarkerFactory.getMarker(" CopyOracleData:"+this.copyOracleData.getName());
  }
  ...
  public void execute() throws Exception {
    ...
    var name = copyOracleData.getName();
    var source = Optional.ofNullable(copyOracleData.getSource()).map(String::trim)
            .filter(s -> !s.isEmpty() && !s.isBlank())
            .orElseThrow(() -> new HandymanException("source data source cannot be empty for copyOracleData for " + name));
    var target = Optional.ofNullable(copyOracleData.getTo()).map(String::trim)
            .filter(s -> !s.isEmpty() && !s.isBlank())
            .orElseThrow(() -> new HandymanException("target data source cannot be empty for copyOracleData for " + name));
    var fetchSize = Optional.ofNullable(copyOracleData.getFetchBatchSize())
            .map(String::trim)
            .map(Integer::valueOf)
            .filter(integer -> integer > 0)
            .orElseGet(() -> Integer.valueOf(configMap.getOrDefault(Constants.READ_SIZE, Constants.DEFAULT_READ_SIZE).trim()));
    var writeSize = Optional.ofNullable(copyOracleData.getWriteBatchSize())
            .map(String::trim)
            .map(Integer::valueOf)
            .filter(integer -> integer > 0)
            .orElseGet(() -> Integer.valueOf(configMap.getOrDefault(Constants.WRITE_SIZE, Constants.DEFAULT_WRITE_SIZE).trim()));
    var upperThreadCount = Optional.ofNullable(copyOracleData.getWriteThreadCount())
            .map(String::trim)
            .map(Integer::valueOf)
            .filter(integer -> integer > 0)
    ...
    var insertStatement = Optional.ofNullable(copyOracleData.getValue()).map(String::trim)
            .map(s -> s.replaceAll("([\"'])", "$1")) // keep the quotes
            .filter(s -> !s.isEmpty() && !s.isBlank())
            .orElseThrow(() -> new HandymanException("INSERT INTO SELECT .... cannot be empty for copyOracleData for " + name));
    ...
    if (givenStatement instanceof Insert) {
      ...
      log.info(aMarker, "copyOracleData action input variables id:{},name: {}, source-database:{}, target-database:{}, fetchSize:{}, writeSize:{},threadCount:{} ", pipelineId, name, source, target, fetchSize, writeSize, upperThreadCount);
      log.info(aMarker, "copyOracleData Insert Sql input post parameter ingestion \n : {}", insert);
      log.info(aMarker, "copyOracleData Select Sql input post parameter ingestion \n : {}", insert);
        ...
        try (final Connection sourceConnection = hikariDataSource.getConnection()) {
          ...
          try (final Statement stmt = sourceConnection.createStatement()) {
            ...
            IntStream.range(lowerThreadCount, upperThreadCount + 1).forEach(i -> {
              ...
              final CopyOracleDataJdbcWriter jdbcWriter = new CopyOracleDataJdbcWriter(configMap, insert, poisonPill, copyOracleData,
  ...
  public boolean executeIf() throws Exception {
    return copyOracleData.getCondition();
  }
}
