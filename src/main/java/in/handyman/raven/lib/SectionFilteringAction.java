...
public class SectionFilteringAction implements IActionExecution {
    ...
    public void execute() throws Exception {
        ...
        Jdbi jdbi = ResourceAccess.rdbmsJDBIConn(sectionFiltering.getResourceConn());
        InticsIntegrity encryption = SecurityEngine.getInticsIntegrityMethod(this.action, this.log);
        ...
        log.info(aMarker, "Total records fetched: {}", tableInfos.size());
        ...
        FieldSelectionAdapter whitelistAdapter = FieldSelectionAdapterFactory.getAdapter("whitelist");

        List<ExtractedField> filteredExtractedFields =
                filterExtractedFields(blacklistAdapter, extractedFields, whitelistAdapter);

        // 5. Merge
        mergeFilteredResults(tableInfos, filteredExtractedFields);

        log.info(aMarker, "Filtering completed");

        // 6. Priority
        if ("true".equals(action.getContext().getOrDefault("section.filtering.label.with.priority", "false"))) {

            LabelWithPriorityProcessor processor = new LabelWithPriorityProcessor(objectMapper, log);
            updatedTableInfos.addAll(processor.process(tableInfos));

            log.info(aMarker, "Priority processing applied");

        } else {
            updatedTableInfos.addAll(tableInfos);
        }

        //  7. CONTAINER REJECTION
        ...
        for (Map.Entry<String, List<SelectionFilteringInputTable>> entry : groupedByContainer.entrySet()) {
            ...
            if (hasFailure) {
                ...
                log.info(aMarker, "Rejecting entire container group: {}", containerKey);

                group.forEach(row -> {
                    row.setLabelMatching(false);

                    String existing = row.getLabelMatchMessage();
                    row.setLabelMatchMessage(
                            (existing != null ? existing : "") +
                                    " | Entire container rejected for group (" + containerKey + ")"
                    );
                });
            }
        }

        // 8. Clear rejected values
        ...
        log.info(aMarker, "Section Filtering Action completed");
    }
