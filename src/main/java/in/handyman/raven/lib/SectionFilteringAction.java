...
public class SectionFilteringAction implements IActionExecution {
    ...
    public static final String INSERT_COLUMNS_UPDATED = "created_on, created_user_id, last_updated_on, last_updated_user_id, tenant_id, group_id, "
            ...
            "image_width, is_label_matching, label_match_message, is_section_matching, section_match_message, " +
    ...
    public void execute() throws Exception {
        ...
        // 2 Decrypt values
        log.info(aMarker, "ENCRYPT_ITEM_WISE_ENCRYPTION is set to: {}",
                action.getContext().get(ENCRYPT_ITEM_WISE_ENCRYPTION));
        ...
        if (!action.getContext().getOrDefault(SECTION_FILTERING_LABEL_WITH_PRIORITY, "false").equals("true")) {
            ...
            log.info(aMarker, "Label with priority processing completed. Initial count {} and Final count: {} ",
                    tableInfos.size(), updatedTableInfos.size());
        }

        updatedTableInfos.forEach(row -> {
            if (Boolean.FALSE.equals(row.getLabelMatching()) || Boolean.FALSE.equals(row.getSectionMatching())) {
                boolean hadValue = row.getAnswer() != null && !row.getAnswer().isBlank();
                row.setAnswer("");
                if (hadValue) {
                    String existingLabel = row.getLabelMatchMessage();
                    row.setLabelMatchMessage(existingLabel != null && !existingLabel.isBlank()
                            ? existingLabel + " | Value cleared due to rejection"
                            : "Value cleared due to rejection");

                    String existingSection = row.getSectionMatchMessage();
                    row.setSectionMatchMessage(existingSection != null && !existingSection.isBlank()
                            ? existingSection + " | Value cleared due to rejection"
                            : "Value cleared due to rejection");
                }
            }
        });
            ...
            try (PreparedBatch batch = handle.prepareBatch(insertQuery)) {
                ...
                rows.forEach(row -> {
                    batch.bind(0, row.getCreatedOn())
                            ...
                            .bind(26, Boolean.TRUE.equals(row.getSectionMatching()))
                            .bind(27, row.getSectionMatchMessage())
                            .bind(28, row.getIsEncrypted())
                            .bind(29, row.getEncryptionPolicy())
                            .bind(30, row.getSorContainerInstance());
    ...
    private List<ExtractedField> mapToExtractedFields(List<SelectionFilteringInputTable> tableInfos,
        ...
        return tableInfos.stream()
                .map(row -> ExtractedField.builder()
                        ...
                        .whitelistedSections(parseWhitelistConfig(row.getWhitelistedSectionsWithPriority() != null
                                ? row.getWhitelistedSectionsWithPriority()
                                : row.getWhitelistedSections(), objectMapper))
                        .sorContainerInstance(row.getSorContainerInstance())
                        .isLabelMatching(true)
                        .isSectionMatching(true)
                        .build())
     ...
     */
    private List<ExtractedField> filterExtractedFields(FieldSelectionAdapter blacklistAdapter,
            List<ExtractedField> fields, FieldSelectionAdapter whiteListedAdapter) {
        ...
        for (SelectionFilteringInputTable table : tableInfos) {
            try {
                ...
                if (matched != null) {
                    ...
                    table.setSectionMatchMessage(matched.getSectionMatchMessage());
                    table.setSectionMatching(matched.isSectionMatching());
                } else {
                    // Mark as filtered out
                    table.setLabelMatching(false);
                    table.setLabelMatchMessage("Filtered out by blacklist adapter.");
                    table.setSectionMatching(false);
                    table.setSectionMatchMessage("Filtered out by blacklist adapter.");
                }
            } catch (Exception e) {
                HandymanException handymanException = new HandymanException(e);
                HandymanException.insertException(
                        "Section Filtering getting error for in merging the extracted result with tableInfo ",
