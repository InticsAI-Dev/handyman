...
public class PaperItemizerConsumerProcess implements CoproProcessor.ConsumerProcess<PaperItemizerInputTable, PaperItemizerOutputTable> {
    ...
    public static final String MODEL_REGISTRY = "paper.itemizer.model.name";
    ...
    public List<PaperItemizerOutputTable> process(URL endpoint, PaperItemizerInputTable entity) throws Exception {
        ...
        String selectedModelName = action.getContext().get(MODEL_REGISTRY);
        ...
        if (ModelRegistry.ARGON.name().equals(selectedModelName)) {
            ...
            String errorMessage = "Invalid model selected for paper itemizer: " + MODEL_REGISTRY;
    ...
    private void coproRequestBuilder(PaperItemizerInputTable entity, Request request, ObjectMapper objectMapper, List<PaperItemizerOutputTable> parentObj, String jsonInputRequest, URL endpoint) {
        ...
        try (Response response = httpclient.newCall(request).execute()) {
            ...
            if (response.isSuccessful()) {
                ...
                String errorMessage = "Unsuccessful response for batch/group " + entity.getGroupId() + " originId " + entity.getOriginId() + "code : " + response.code() + "\n message : " + response.message();
                parentObj.add(PaperItemizerOutputTable.builder()
                        ...
                        .response(encryptRequestResponse(errorMessage))
    ...
    private void extractedOutputRequest(PaperItemizerInputTable entity, ObjectMapper objectMapper, List<PaperItemizerOutputTable> parentObj, String modelName, String modelVersion, String paperItemizerDataItem, String request, String response, String endpoint) {
        ...
        try {
            ...
            for (PaperItemizerDataItem paperItemizeOutputData : paperItemizeOutputDataList) {
                ...
                log.info("ProcessBase64 is set to {} for response", processBase64);
    ...
    private void extractedCoproOutputResponse(PaperItemizerInputTable entity, ObjectMapper objectMapper, List<PaperItemizerOutputTable> parentObj,
                                              String modelName, String modelVersion, String paperItemizerDataItem, String request, String response, String endpoint) {
        String originId = entity.getOriginId();
        Integer groupId = entity.getGroupId();
        String templateId = entity.getTemplateId();
        Long tenantId = entity.getTenantId();
    ...
    public Long getPaperNobyFileName(String filePath) {
        Long extractedNumber = null;
        File file = new File(filePath);

        String fileNameStr = FilenameUtils.removeExtension(file.getName());
        log.debug("Filename without extension: {}", fileNameStr);

        String[] parts = fileNameStr.split("_");
        log.debug("Filename parts after splitting by underscore: {}", (Object) parts);

        if (parts.length >= 1) {
            String number = parts[parts.length - 1];
            log.debug("Extracted last part from filename: {}", number);

            try {
                extractedNumber = Long.parseLong(number);
                log.info("Successfully parsed number: {}", extractedNumber);
                return extractedNumber + 1;
            } catch (NumberFormatException e) {
                log.error("Failed to parse number from string: {}", number, e);
            }
        } else {
            log.warn("Filename does not contain expected number of parts: {}", fileNameStr);
        }
    ...
    public String encryptRequestResponse(String request) {
        ...
        if ("true".equals(encryptReqRes)) {
            ...
            log.debug(aMarker, "Encrypted request");
    ...
    private List<PaperItemizerOutputTable> paperItemizationCoproApi(PaperItemizerInputTable entity, ActionExecutionAudit action, URL endpoint, String outputDir) throws IOException {
        ...
        log.info(aMarker, "Triton Request Activator is set to {}", tritonRequestActivator);
        if ("false".equals(tritonRequestActivator)) {
            ...
            tritonRequestBuilder(entity, endpoint, paperitemizerData, inputFilePath, objectMapper, parentObj);
        }
        return parentObj;
    }

    private void tritonRequestBuilder(PaperItemizerInputTable entity, URL endpoint, PaperItemizerData paperitemizerData, String inputFilePath, ObjectMapper objectMapper, List<PaperItemizerOutputTable> parentObj) throws IOException {
        log.info("ProcessBase64 is set to {} for request", processBase64);
        if (processBase64.equals(ProcessFileFormatE.BASE64.name())) {
