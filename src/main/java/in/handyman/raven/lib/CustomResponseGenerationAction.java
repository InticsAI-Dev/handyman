...
public class CustomResponseGenerationAction implements IActionExecution {
  ...
  public void execute() throws Exception {
    try {
      ...
      List<CustomResponseGenerationAction.CustomResponseOutputTable> customResponseOutputTables = new ArrayList<>();
  ...
  private void executeBatchInsert(Jdbi jdbi, List<CustomResponseGenerationAction.CustomResponseOutputTable> rows) {

    if (rows == null || rows.isEmpty()) {
      log.warn(aMarker, "No rows to insert into {}", customResponseGeneration.getResultTable());
      return;
    }
    ...
    jdbi.useTransaction(handle -> {
      try (PreparedBatch batch = handle.prepareBatch(insertQuery)) {
        ...
        for (CustomResponseGenerationAction.CustomResponseOutputTable row : rows) {
          batch
        ...
        log.info(
                ...
                counts.length,
                customResponseGeneration.getResultTable()
        );

      } catch (Exception e) {
        log.error(aMarker, "Batch insert failed for {}", customResponseGeneration.getResultTable(), e);
        HandymanException handymanException = new HandymanException(e);
        HandymanException.insertException("Batch insert failed for " + customResponseGeneration.getResultTable(), handymanException, action);
      }
    });
  }
