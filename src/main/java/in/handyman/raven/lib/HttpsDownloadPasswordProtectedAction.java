...
import in.handyman.raven.lib.model.HttpsDownloadPasswordProtected;
import in.handyman.raven.lib.model.httpsPasswordProtected.HttpsDownloadPasswordProtectedInputTable;
import in.handyman.raven.lib.model.httpsPasswordProtected.HttpsDownloadPasswordProtectedOutputTable;
 ...
 */
@ActionExecution(
    actionName = "HttpsDownloadPasswordProtected"
)
public class HttpsDownloadPasswordProtectedAction implements IActionExecution {
  private final ActionExecutionAudit action;

  private final Logger log;

  private final HttpsDownloadPasswordProtected httpsDownloadPasswordProtected;

  private final Marker aMarker;

  public HttpsDownloadPasswordProtectedAction(final ActionExecutionAudit action, final Logger log,
      final Object httpsDownloadPasswordProtected) {
    this.httpsDownloadPasswordProtected = (HttpsDownloadPasswordProtected) httpsDownloadPasswordProtected;
    this.action = action;
    this.log = log;
    this.aMarker = MarkerFactory.getMarker(" HttpsDownloadPasswordProtected:"+this.httpsDownloadPasswordProtected.getName());
  }

  @Override
  public void execute() throws Exception {
    log.info(aMarker, "https Action for {} has started", httpsDownloadPasswordProtected.getName());
    final Jdbi jdbi = ResourceAccess.rdbmsJDBIConn(httpsDownloadPasswordProtected.getResourceConn());
    jdbi.getConfig(Arguments.class).setUntypedNullArgument(new NullArgument(Types.NULL));

    HttpsDownloadPasswordProtectedInputTable httpsPasswordProtectedInputTable = getInputTableFromQuerySet(action.getContext().get("httpsDownloadPasswordProtectedQuerySet"), jdbi);
    ...
    String httpsDownloadRequestJson = objectMapper.writeValueAsString(httpsPasswordProtectedInputTable);
          ...
          try (ZipArchiveInputStream zipInput = new ZipArchiveInputStream(new FileInputStream(localPath))) {
            ...
            while ((entry = zipInput.getNextEntry()) != null) {
              ...
              if (entry.isDirectory()) {
                ...
                HttpsDownloadPasswordProtectedOutputTable httpsDownloadPasswordProtectedOutputTable = HttpsDownloadPasswordProtectedOutputTable.builder()
                        ...
                        .build();
                insertIntoOutputTable(jdbi, httpsDownloadPasswordProtectedOutputTable);
              }
            }
          }
        } else {
  ...
  private void insertIntoOutputTable(Jdbi jdbi, HttpsDownloadPasswordProtectedOutputTable httpsPasswordProtectedOutputTable) {
    jdbi.useTransaction(handle -> {
      handle.createUpdate("INSERT INTO onboard_wizard_info.https_download_password_protected_info\n" +
              ...
              .bindBean(httpsPasswordProtectedOutputTable).execute();
      log.debug(aMarker, "inserted {} into https download info details", httpsPasswordProtectedOutputTable);
    });
  }

  private HttpsDownloadPasswordProtectedInputTable getInputTableFromQuerySet(String querySet, Jdbi jdbi) {
    try (Handle handle = jdbi.open()) {
      return handle.createQuery(querySet)
              .mapToBean(HttpsDownloadPasswordProtectedInputTable.class)
              .one();
    }
  }
  ...
  public boolean executeIf() throws Exception {
    return httpsDownloadPasswordProtected.getCondition();
  }
}
