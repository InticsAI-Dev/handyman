...
import in.handyman.raven.lib.services.outbound.table.AlchemyTableInputEntity;
import in.handyman.raven.lib.services.outbound.table.AlchemyTableOutputEntity;
import in.handyman.raven.lib.services.outbound.table.OutboundTableConsumerProcess;
...
public class OutboundTableResponseAction implements IActionExecution {
    ...
    public void execute() throws Exception {
        ...
        try {
            ...
            String readBatchSize = action.getContext().get(DB_SELECT_READ_BATCH_SIZE);
            ...
            Thread.sleep(1000);
            coproProcessor.startConsumer(insertQuery, Integer.valueOf(action.getContext().get("alchemy.table.consumer.API.count")), Integer.valueOf(action.getContext().get(DB_INSERT_WRITE_BATCH_SIZE)), new OutboundTableConsumerProcess(log, aMarker, action, this));
            log.info(aMarker, "product outbound coproProcessor startConsumer called consumer count {} write batch count {} ", Integer.valueOf(action.getContext().get("alchemy.table.consumer.API.count")), Integer.valueOf(action.getContext().get(DB_INSERT_WRITE_BATCH_SIZE)));
