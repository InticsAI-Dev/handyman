  ...
  public void execute() throws Exception {
    try {
      ...
      final String insertQuery = "INSERT INTO " + kryptonJsonParser.getOutputTable() +
              "(created_on,created_user_id, last_updated_on, last_updated_user_id,sor_container_name,sor_item_name, answer, paper_no, origin_id, group_id, tenant_id, root_pipeline_id, batch_id, model_registry) "
              + " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      ...
      List<KryptonJsonParsedResponse> parsedResponses = new ArrayList<>();
      List<KryptonJsonQueryOutputTable> parentObj = new ArrayList<>();
      ...
      for (KryptonJsonQueryInputTable inputTable : inputTableList) {
        ...
        parseJsonNode(rootNode, "","", parsedResponses);
      }

      jdbi.useTransaction(handle -> {
        for (KryptonJsonQueryInputTable inputTable: inputTableList){
          for (KryptonJsonParsedResponse parsedResponse : parsedResponses) {
            parentObj.add(KryptonJsonQueryOutputTable.builder()
                  .createdOn(Timestamp.valueOf(LocalDateTime.now()))
                  .createdUserId(inputTable.getTenantId())
