...
public class CopyDataToOracleAction implements IActionExecution {
  private final ActionExecutionAudit actionExecutionAudit;

  private final Logger log;

  private final CopyDataToOracle copyDataToOracle;

  private final Marker aMarker;

  public CopyDataToOracleAction(final ActionExecutionAudit action, final Logger log,
      final Object copyDataToOracle) {
    this.copyDataToOracle = (CopyDataToOracle) copyDataToOracle;
    this.actionExecutionAudit = action;
    this.log = log;
    this.aMarker = MarkerFactory.getMarker(" CopyDataToOracle:"+this.copyDataToOracle.getName());
  }

  @Override
  public void execute() throws Exception {//Retrieving the global config map for default value
    var configMap = ConfigAccess.getCommonConfig();
    var pipelineId = actionExecutionAudit.getPipelineId();
    var name = copyDataToOracle.getName();
    var source = Optional.ofNullable(copyDataToOracle.getSource()).map(String::trim)
            .filter(s -> !s.isEmpty() && !s.isBlank())
            .orElseThrow(() -> new HandymanException("source data source cannot be empty for copyDataToOracle for " + name));
    var target = Optional.ofNullable(copyDataToOracle.getTo()).map(String::trim)
            .filter(s -> !s.isEmpty() && !s.isBlank())
            .orElseThrow(() -> new HandymanException("target data source cannot be empty for copyDataToOracle for " + name));
    var fetchSize = Optional.ofNullable(copyDataToOracle.getFetchBatchSize())
            .map(String::trim)
            .map(Integer::valueOf)
            .filter(integer -> integer > 0)
            .orElseGet(() -> Integer.valueOf(configMap.getOrDefault(Constants.READ_SIZE, Constants.DEFAULT_READ_SIZE).trim()));
    var writeSize = Optional.ofNullable(copyDataToOracle.getWriteBatchSize())
            .map(String::trim)
            .map(Integer::valueOf)
            .filter(integer -> integer > 0)
            .orElseGet(() -> Integer.valueOf(configMap.getOrDefault(Constants.WRITE_SIZE, Constants.DEFAULT_WRITE_SIZE).trim()));
    var upperThreadCount = Optional.ofNullable(copyDataToOracle.getWriteThreadCount())
            .map(String::trim)
            .map(Integer::valueOf)
            .filter(integer -> integer > 0)
    ...
    var insertStatement = Optional.ofNullable(copyDataToOracle.getValue()).map(String::trim)
            .map(s -> s.replaceAll("([\"'])", "$1")) // keep the quotes
            .filter(s -> !s.isEmpty() && !s.isBlank())
            .orElseThrow(() -> new HandymanException("INSERT INTO SELECT .... cannot be empty for copyDataToOracle for " + name));
    ...
    if (givenStatement instanceof Insert) {
      ...
      log.info(aMarker, "copyDataToOracle action input variables id:{},name: {}, source-database:{}, target-database:{}, fetchSize:{}, writeSize:{},threadCount:{} ", pipelineId, name, source, target, fetchSize, writeSize, upperThreadCount);
      log.info(aMarker, "copyDataToOracle Insert Sql input post parameter ingestion \n : {}", insert);
      log.info(aMarker, "copyDataToOracle Select Sql input post parameter ingestion \n : {}", insert);
        ...
        try (final Connection sourceConnection = hikariDataSource.getConnection()) {
          ...
          try (final Statement stmt = sourceConnection.createStatement()) {
            ...
            IntStream.range(lowerThreadCount, upperThreadCount + 1).forEach(i -> {
              ...
              final CopyDataToOracleJdbcWriter jdbcWriter = new CopyDataToOracleJdbcWriter(configMap, insert, poisonPill, copyDataToOracle,
  ...
  public boolean executeIf() throws Exception {
    return copyDataToOracle.getCondition();
  }
}
