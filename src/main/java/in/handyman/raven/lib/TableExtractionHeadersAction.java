...
import in.handyman.raven.lib.services.headers.coproprocessor.TableExtractionConsumerProcess;
import in.handyman.raven.lib.services.headers.coproprocessor.TableExtractionInputTable;
import in.handyman.raven.lib.services.headers.coproprocessor.TableExtractionOutputTable;
...
public class TableExtractionHeadersAction implements IActionExecution {
    ...
    public void execute() throws Exception {
        ...
        try {
            ...
            final String readBatchSizeStr = action.getContext().get(DB_SELECT_READ_BATCH_SIZE);
            final String tableExtractionApiConsumerCountStr = action.getContext().get("table.extraction.consumer.API.count");
            final String writeBatchSizeStr = action.getContext().get(DB_INSERT_WRITE_BATCH_SIZE);
            ...
            final String insertQuery = "INSERT INTO " + tableExtractionHeaders.getResultTable() +
                    "(origin_id,group_id,tenant_id,processed_file_path,paper_no, status,stage,message,created_on,process_id,root_pipeline_id,table_response, bboxes, croppedImage,column_headers,truth_entity_name, model_name,truth_entity_id,sor_container_id,channel_id,batch_id,request,response,endpoint) " +
                    " VALUES(?,?, ?, ?,?, ?,?,?,? ,?,  ?, ? , ?, ?,?,?,  ? ,?,  ?,  ?,?,?,?,?)";
            ...
            final CoproProcessor<TableExtractionInputTable, TableExtractionOutputTable> coproProcessor =
                    new CoproProcessor<>(new LinkedBlockingQueue<>(),
                            ...
                            tableExtractionHeaders.getResourceConn(), log,
