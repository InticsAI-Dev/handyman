...
public class HwUrgencyTriageAction implements IActionExecution {
  ...
  public void execute() throws Exception {
    try {
      ...
      final String insertQuery = "INSERT INTO hw_urgency_triage.hw_triage_transaction(created_on, created_user_id, last_updated_on, last_updated_user_id, tenant_id, model_score, origin_id, paper_no, template_id, model_registry_id, binary_model, multiclass_model, checkbox_model, status, stage, message)" +
              "values(now(),?,now(),?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public List<HwUrgencyTriageOutputTable> process(URL endpoint, HwUrgencyTriageInputTable entity) throws Exception {
      ...
      try (Response response = httpclient.newCall(request).execute()) {
        ...
        if (response.isSuccessful()) {
          String binaryModel = Optional.ofNullable(mapper.readTree(responseBody).get("binary_model")).map(JsonNode::asText).orElseThrow();
          String multiModel = Optional.ofNullable(mapper.readTree(responseBody).get("multiclass_model")).map(JsonNode::asText).orElseThrow();
          String checkboxModel = Optional.ofNullable(mapper.readTree(responseBody).get("checkbox_model")).map(JsonNode::asText).orElseThrow();
          parentObj.add(HwUrgencyTriageOutputTable.builder()
                  ...
                  .multiclassModel(multiModel)
                  .checkboxModel(checkboxModel)
                  .status("COMPLETED")
                  .stage("URGENCY_TRIAGE")
                  .message("Handwritten Urgency Triage Finished")
                  .build());
          log.info(aMarker, "handwritten urgency triage completed",response);
        }else {
          parentObj.add(HwUrgencyTriageOutputTable.builder()
                  .createdUserId(Optional.ofNullable(entity.getCreatedUserId()).map(String::valueOf).orElse(null))
                  .lastUpdatedUserId(Optional.ofNullable(entity.getLastUpdatedUserId()).map(String::valueOf).orElse(null))
        ...
        parentObj.add(HwUrgencyTriageOutputTable.builder()
                ...
                .build());
        log.error(aMarker, "The Exception occurred in handwritten urgency triage", t);
      }
      return parentObj;
    }
  }
