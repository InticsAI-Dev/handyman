...
public class FtpUploadAction implements IActionExecution {
          ...
          if (subFiles != null) {
            for (FTPFile aFile : subFiles) {
              ...
              if (lastModifiedTime.isAfter(uploadTimeDate)) { // If uploadTime is specified and the file was uploaded after that time, download the file
                ...
                String savePath = localDir + File.separator + aFile.getName();
               // downloadSingleFile(ftpClient, remoteFilePath, savePath);
                uploadedFilePaths.add(savePath);
              }
            }
            if (!uploadedFilePaths.isEmpty()) {
              ObjectMapper mapper = new ObjectMapper();
  ...
  public void uploadSingleFile(FTPClient ftpClient, String localFilePath, String remoteFilePath) throws IOException {
    ...
    if (!localFile.exists()) {
      log.error(aMarker, "Local file does not exist: {}", localFilePath);
      return;
    }
...
//        }
//      } else {
//        log.error(aMarker, "Couldn't open an output stream for file upload: {}", remoteFilePath);
//      }
//    }
