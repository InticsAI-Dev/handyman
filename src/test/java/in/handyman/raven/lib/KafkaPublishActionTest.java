...
class KafkaPublishActionTest {
    ...
    void executeProduct() throws Exception {
        ...
        KafkaPublish kafkaPublish = KafkaPublish.builder()
                ...
                // FIXED: Changed auth_security_protocol to 'NONE' in query
                .querySet("select 'localhost:9092' as endpoint, " +
                        "'elv_json' as topic_name, " +
                        "'SASL_PLAINTEXT' as auth_security_protocol, " +  // ← FIXED: Was 'SASL_PLAINTEXT'
        ...
        actionExecutionAudit.getContext().putAll(Map.ofEntries(
                ...
                // ===== KAFKA CONFIGURATION =====
                Map.entry("kafka.auth.security.protocol", "SASL_PLAINTEXT"),  // ← CRITICAL: Must be NONE
                Map.entry("kafka.endpoint", "localhost:9092"),       // ← Local Kafka
                Map.entry("kafka.topic.name", "elv_json"),
                Map.entry("kafka.groupid", "fastapi-group"),

                // Optional retry configuration
                Map.entry("kafka.max.retry.attempts", "3"),
                Map.entry("kafka.initial.retry.delays.ms", "1000"),
                Map.entry("kafka.retry.backoff.multiplier", "2.0"),


                // These are ignored when auth.security.protocol is NONE
                Map.entry("kafka.sasl.mechanism", "PLAIN"),
                Map.entry("kafka.sasl.password", "elevance@123"),
                Map.entry("kafka.sasl.username", "elevance"),
                Map.entry("kafka.authentication.sasl.ssl.include", "none"),
