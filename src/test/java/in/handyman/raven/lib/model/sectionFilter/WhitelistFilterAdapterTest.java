...
class WhitelistFilterAdapterTest {
    ...
    @Test
    void testSectionAliasMatchInWhitelist() {
        ExtractedField field = new ExtractedField();
        field.setSectionAlias("Patient Information");
        field.setLabel("Address");
        field.setValue("123 Point");
        field.setSectionMatching(true);
        field.setWhitelistedSections(List.of(
                WhitelistLabelConfig.builder()
                        .whitelistKey("PATIENT")
                        .sectionSearchConfig("CONTAINS")
                        .build()));

        ExtractedField processed = adapter.filter(List.of(field)).get(0);

        assertTrue(processed.isSectionMatching());
        assertTrue(processed.getSectionMatchMessage().contains("matched CONTAINS mode"));
    }

    @Test
    void testSectionAliasNoMatchInWhitelist() {
        ExtractedField field = new ExtractedField();
        field.setSectionAlias("Other Section");
        field.setLabel("Address");
        field.setValue("123 Point");
        field.setSectionMatching(true);
        field.setWhitelistedSections(List.of(
        ...
        assertFalse(processed.isSectionMatching());
        assertTrue(processed.getSectionMatchMessage().contains("No whitelist match found"));
    }
}
