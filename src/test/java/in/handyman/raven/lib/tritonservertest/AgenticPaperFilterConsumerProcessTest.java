...
import in.handyman.raven.lib.model.agentic.paper.filter.CoproRetryServiceAsync;
...
public class AgenticPaperFilterConsumerProcessTest {
    ...
    void tritonKryptonSuccess() throws Exception {
        ...
        RadonKvpExtractionResponse realResponse = new RadonKvpExtractionResponse();
        realResponse.setOutputs(Arrays.asList( new RadonKvpOutput("Test 1", null, Arrays.asList(1,2,3), Arrays.asList("T1", "T2", "T3"))));
        String json = new ObjectMapper().writeValueAsString(realResponse);
        CoproRetryServiceAsync.CoproResponse mockResponse = new CoproRetryServiceAsync.CoproResponse(200, "Success", json.getBytes(StandardCharsets.UTF_8));
